...
on setup_globals(caller)
	...
	try
		...
		set Code_version_epoch to "1777613450" -- Updated: 2026-05-01 05:30:50 UTC
	on error errmsg
		return false
	end try
	return true
...
on setup_logging(caller)
	...
	try
		...
		set Logger_levels_all to {"INFO", "WARN", "ERROR", "NEAT", "FATAL", "DEBUG", "TRACE", "JSON", "BARS"}
...
on run {}
	...
	if startup_success is true then
		...
		my read_data(cmi)
		my logger(true, handlername, caller, "WARN", "Guide_hours: " & Guide_hours)
...
on idle
	...
	if "BARS" is in Logger_levels then
		...
		my logger(true, handlername, caller, "INFO", "Guide_hours: " & Guide_hours)
		delay 0.1
	end if
	try
							...
							if show_next of item i of Show_info is less than or equal to cd_object then
								if show_recording of item i of Show_info is false then
									...
									if tunermax of tuner_status_result is greater than tuneractive of tuner_status_result then
										...
										if item 2 of my showid2PID(cm, show_id of item i of Show_info, false, true) is {} then
											...
											if (show_fail_count of item i of Show_info) is less than Fail_count then
												display notification "Ends " & short_date(cm, show_end of item i of Show_info, false, false) of LibScript with title Recordsoon_icon of Icon_record & " Started Recording on (" & hdhr_record of item i of Show_info & ")" subtitle quote & show_title of item i of Show_info & quote & " on " & show_channel of item i of Show_info & " (" & my channel2name(cm, show_channel of item i of Show_info as text, hdhr_record of item i of Show_info) & ")"
									...
									if notify_recording_time of item i of Show_info is less than (cd) or notify_recording_time of item i of Show_info is missing value then
										...
										my logger(true, handlername, caller, "INFO", "Recording in progress for " & quote & (show_title of item i of Show_info & quote & " on " & show_channel of item i of Show_info & ", ends in " & ms2time("idle_rip(19.1)", (show_end of item i of Show_info) - (cd), "s", 3) of LibScript) & ", Next Update: " & fixDate(cm, time string of (notify_recording_time of item i of Show_info)) of LibScript)
										
										my tuner_inuse(cm, hdhr_record of item i of Show_info)
										update_folder(cm, show_dir of item i of Show_info) of LibScript
										--set notify_recording_time of item i of Show_info to (cd) + (Notify_recording * minutes)
									end if
							...
							if (show_end of item i of Show_info) is less than or equal to cd then
								...
								try
									if (temp_OriginalAirdate) is not in {"", {}} then
										...
										set temp_dateobject to epoch2datetime(cm, temp_OriginalAirdate) of LibScript
										...
										try
											if show_recording_path of item i of Show_info is not in {missing value, {}, ""} then
												date2touch(cm, temp_dateobject, show_recording_path of item i of Show_info) of LibScript
								...
								if show_is_series of item i of Show_info is true then
									if show_use_seriesid of item i of Show_info is false then
										...
										display notification "Next Showing: " & short_date(cm, show_next of item i of Show_info, false, false) of LibScript with title Stop_icon of Icon_record & " Recording Complete" subtitle (quote & show_title of item i of Show_info & quote & " on " & show_channel of item i of Show_info & " (" & my channel2name(cm, show_channel of item i of Show_info as text, hdhr_record of item i of Show_info) & ")")
									else
										set show_fail_count of item i of Show_info to 0
										set show_fail_reason of item i of Show_info to ""
										set show_recorded_today of item i of Show_info to true
	...
	try
		...
		if hdhr_quit_record is true then
			...
			if systemShutdown is false then
				...
				set quit_response to button returned of (display dialog "Do you want to cancel these recordings already in progress?" & return & return & stringlistflip(my cm(handlername, caller), hdhr_quit_record_titles, return, "string") of LibScript buttons {"Go Back", "Yes", "No"} default button 3 with title my check_version_dialog(my cm(handlername, caller)) giving up after Dialog_timeout with icon caution)
...
on hdhrGRID(caller, hdhr_device, hdhr_channel)
	...
	repeat with i from 1 to length of Guide of hdhrGRID_temp
		set temp_name to show_name_fix(my cm(handlername, caller), "", item i of Guide of hdhrGRID_temp) of LibScript
		...
		if the_show_id of show_status is not missing value then
			...
			else if sec_to_show is less than 0 then
				set temp_status_icon to Warning_icon of Icon_record
			else if sec_to_show is less than 1 * hours then
				set temp_status_icon to Film_icon of Icon_record
			else if sec_to_show is less than 4 * hours then
				set temp_status_icon to Up_icon of Icon_record
			else if show_stat of show_status is "Recorded today" then
				set temp_status_icon to Check_icon of Icon_record
			else
				set temp_status_icon to Futureshow_icon of Icon_record
			end if
		end if
	...
	if epoch2datetime(my cm(handlername, caller), EndTime of item 1 of list_position_response) of LibScript is less than (current date) then
...
on is_channel_record(caller, hdhr_tuner, channelcheck, cd)
	...
	repeat with i from 1 to length of Show_info
		if channelcheck is show_channel of item i of Show_info and hdhr_tuner is hdhr_record of item i of Show_info then
			if (weekday of (cd) as text) is in show_air_date of item i of Show_info then
				set sec_to_show to ((show_next of item i of Show_info) - (cd))
				if show_active of item i of Show_info is true then
					if show_recording of item i of Show_info is true then
						set end of temp_show_line to Record_icon of Icon_record
					else
						if sec_to_show is less than 4 * hours then
							if sec_to_show is less than 0 then
								set end of temp_show_line to Warning_icon of Icon_record
							else if sec_to_show is less than 1 * hours then
								set end of temp_show_line to Film_icon of Icon_record
							else if sec_to_show is less than 4 * hours then
							...
							if (date string of cd) is (date string of (show_next of item i of Show_info)) then
								...
								set end of temp_show_line to Futureshow_icon of Icon_record
							end if
						end if
					end if
				else
	...
	set icon_string to ""
	repeat with icon in temp_show_line
		set icon_string to icon_string & icon
	end repeat
	return icon_string
...
on show_info_dump(caller, show_id_lookup, userdisplay)
	...
	set i to HDHRShowSearch(my cm(handlername, caller), show_id_lookup) of LibScript
	if Local_env is not in Debugger_apps then
...
on channel_list_position(caller, channel_number, channel_list)
	...
	try
		...
		set channel_offset to list_position(my cm(handlername, caller), channel_number, channel_list, false) of LibScript
...
on nextday(caller, the_show_id)
	...
	set show_offset to HDHRShowSearch(my cm(handlername, caller), the_show_id) of LibScript
	...
	repeat with i from -1 to 7
		if ((weekday of (cd_object + i * days)) as text) is in (show_air_date of item show_offset of Show_info) then
			...
			if cd_object is less than (time_set(my cm(handlername, caller), (cd_object + i * days), (show_time of item show_offset of Show_info)) of LibScript) + ((show_length of item show_offset of Show_info) * minutes) then
				...
				set nextup to time_set(my cm(handlername, caller), (cd_object + i * days), show_time of item show_offset of Show_info) of LibScript
...
on validate_show_info(caller, show_to_check, should_edit)
	...
	if show_to_check is "" then
		...
		set i to HDHRShowSearch(my cm(handlername, caller), show_to_check) of LibScript
		...
		if should_edit is true then
			if show_active of item i of Show_info is true then
				...
				if button returned of show_deactivate contains "Deactivate" then
					set show_active of item i of Show_info to false
					set show_recording of item i of Show_info to false
		...
		if show_active of item i of Show_info is true and show_active_changed is false then
			if show_title of item i of Show_info is missing value or show_title of item i of Show_info is "" or should_edit is true then
				...
				if button returned of show_title_temp contains "Series" then
					...
					-- Determine default button based on current state
					...
					if series_type contains "Date/Time" then
						...
						-- SeriesID(Channel): is_series=true, use_seriesid=true, use_seriesid_all=false
						...
						-- SeriesID(All): is_series=true, use_seriesid=true, use_seriesid_all=true
						set show_use_seriesid of item i of Show_info to true
						set show_use_seriesid_all of item i of Show_info to true
						set should_edit to false
					end if
				else if button returned of show_title_temp contains "Single" then
					-- Single: is_series=false, use_seriesid=false, use_seriesid_all=false
					set show_is_series of item i of Show_info to false
					set show_use_seriesid of item i of Show_info to false
					set show_use_seriesid_all of item i of Show_info to false
					set should_edit to true
				end if
				my logger(true, handlername, caller, "INFO", "State: is_series=" & show_is_series of item i of Show_info & ", use_seriesid=" & show_use_seriesid of item i of Show_info & ", use_seriesid_all=" & show_use_seriesid_all of item i of Show_info)
			end if
			
			-- ENFORCE STATE AND PROMPT ACCORDINGLY
			set show_state to ""
			if show_is_series of item i of Show_info is true then
				if show_use_seriesid_all of item i of Show_info is true then
					set show_state to "SeriesID(All)"
				else if show_use_seriesid of item i of Show_info is true then
					set show_state to "SeriesID(Channel)"
				else
					set show_state to "DateTime"
				end if
			else
				set show_state to "Single"
			end if
			my logger(true, handlername, caller, "DEBUG", "Show state: " & show_state)
			...
			if show_state is "SeriesID(All)" or show_state is "SeriesID(Channel)" then
				...
				if show_state is "DateTime" then
					...
					-- For Single: only use first day as default (not all days), require selection
					...
					if (count of show_air_date of item i of Show_info) > 0 then
						...
						set temp_default_days to {item 1 of Full_week_days}
					end if
					set temp_air_date to (choose from list Full_week_days default items temp_default_days with title my check_version_dialog(my cm(handlername, caller)) OK button name "Next.." cancel button name Running_icon of Icon_record & " Run" with prompt "Select the day you wish to record" & return & "This is a single, you can only select 1 day" without empty selection allowed without multiple selections allowed)
				end if
			...
			-- CHANNEL SELECTION
			-- Ask (DateTime/Single/SeriesID(Channel)), Skip (SeriesID(All))
			if show_state is not "SeriesID(All)" and (show_channel of item i of Show_info is missing value or is_number(my cm(handlername, caller), show_channel of item i of Show_info) of LibScript is false) then
			...
			if show_state is "DateTime" or show_state is "Single" then
				if show_time of item i of Show_info is missing value or (show_time of item i of Show_info as number) is greater than or equal to 24 or is_number(my cm(handlername, caller), show_time of item i of Show_info) of LibScript is false or should_edit is true then
			...
			-- Ask (DateTime/Single), Skip (SeriesID modes)
			if show_state is "DateTime" or show_state is "Single" then
				if show_length of item i of Show_info is missing value or is_number(my cm(handlername, caller), show_length of item i of Show_info) of LibScript is false or show_length of item i of Show_info is less than or equal to 0 or should_edit is true then
	...
	repeat 1 times
		try
			if button returned of hdhr_setup_response is "Defaults" then
				...
				try
					my logger(true, handlername, caller, "INFO", "Starting guide hours dialog, Guide_hours = " & Guide_hours)
					set temp to false
					repeat until temp is true
						set guide_length to (display dialog "How many hours of guide data to grab?" & return & "6-24 valid range" default answer Guide_hours buttons {"Run", "6H (Default)", "Set"} default button 3)
			...
			if button returned of hdhr_setup_response is "Logging" then
				...
				if length of logging_response is greater than 0 then
					set Logger_levels to logging_response
					set LoggerLevels of Hdhr_config to logging_response
				end if
			end if
		on error errmsg
...
on main(caller, emulated_button_press)
	...
	if show_info_length is greater than 0 then
		repeat with i from 1 to show_info_length
			if show_last of item i of Show_info is not epoch(cd) of LibScript and show_is_series of item i of Show_info is false then
				set show_active of item i of Show_info to false
			end if
		end repeat
	end if
	...
	if emulated_button_press is not in {"Add", "Shows"} then
		...
		try
			...
			if show_list_empty is true then
				...
				set title_response to (display dialog "Would you like to add a show?" & return & return & "Tuner(s): " & return & stringlistflip(cm, my tuner_overview(cm), return, "string") of LibScript buttons {Tv_icon of Icon_record & " Shows..", Plus_icon of Icon_record & " Add..", Running_icon of Icon_record & " Run"} with title my check_version_dialog(cm) giving up after (Dialog_timeout * 0.5) with icon curl2icon(cm, "https://raw.githubusercontent.com/identd113/hdhr_VCR-AS/master/app.jpg") of LibScript default button 2)
				my logger(true, handlername, caller, "INFO", "EMPTY LIST")
			else
				my logger(true, handlername, caller, "TRACE", "Tracking non open3")
				set title_response to (display dialog "Would you like to add a show?" & return & return & "Tuner(s): " & return & stringlistflip(cm, my tuner_overview(cm), return, "string") of LibScript & return & return & my recordingnow_main(cm) & return & error_shows & return & return & Up_icon of Icon_record & " Next Show: " & next_show_main_time & " (in " & ms2time(cm, (next_show_main_time_real) - (current date), "s", 2) of LibScript & ")" & return & next_show_main buttons {Tv_icon of Icon_record & " Shows..", Plus_icon of Icon_record & " Add..", Running_icon of Icon_record & " Run"} with title my check_version_dialog(cm) giving up after (Dialog_timeout * 0.5) with icon curl2icon(cm, "https://raw.githubusercontent.com/identd113/hdhr_VCR-AS/master/app.jpg") of LibScript default button 2)
	...
	if button returned of title_response contains "Run" or gave up of title_response is true then
		...
		if option_down of isModifierKeyPressed(cm, "option", "Quit?") of LibScript is true then
			quit {}
		end if
		my idle_change(cm, 1, 3)
...
on add_show_info(caller, hdhr_device, hdhr_channel)
	...
	if tunermax of tuner_status_result is tuneractive of tuner_status_result then
		set tuner_status_icon to hdhr_device & " has no available tuners" & return & "Next timeout: " & ms2time(cm, my tuner_ready_time(cm, hdhr_device), "s", 3) of LibScript
	end if
		...
		repeat with i3 from 1 to length of hdhrGRID_response
			repeat 1 times
				...
				if length of hdhrGRID_response is 1 and hdhrGRID_response is {""} then
					...
					repeat until is_number(cm, show_time of temp_show_info) of LibScript and show_time of temp_show_info is greater than or equal to 0 and show_time of temp_show_info is less than 24
						set Time_slide to 0
					...
					repeat until is_number(cm, show_length of temp_show_info) of LibScript and show_length of temp_show_info is greater than or equal to 1
					...
					try
						if imageurl_temp is not "" then
							set temp_icon to curl2icon(cm, imageurl_temp) of LibScript
						else
					...
					try
						set show_url of temp_show_info to add_record_url(cm, show_channel_temp, hdhr_device) of LibScript
					...
					try
						...
						set show_originalairdate_real to short_date(cm, epoch2datetime(cm, show_originalairdate) of LibScript, false, false) of LibScript
					on error errmsg
				...
				if show_is_series of temp_show_info is true and show_use_seriesid of temp_show_info is false then
					...
					if show_air_date of temp_show_info is false then
						my logger(true, handlername, caller, "INFO", "add_show_info EXIT: User cancelled day selection (DateTime series)")
						return
					end if
				...
				if temp_show_dir is missing value then
					...
					-- Build fallback chain: configured default, last show dir, volume root, /Volumes/
					set folder_fallbacks to {}
					try
						set end of folder_fallbacks to Hdhr_setup_folder as alias  -- tier 1: configured default folder
					on error
					end try
					try
						set last_dir to show_dir of last item of Show_info as alias
						set end of folder_fallbacks to last_dir  -- tier 2: last show's folder
						-- Extract volume name: "Raid6:DVR Tests:" → "Raid6"
						set vol_name to text 1 thru ((offset of ":" in (last_dir as text)) - 1) of (last_dir as text)
						set end of folder_fallbacks to alias ("Volumes:" & vol_name & ":")  -- tier 3: volume root of last show
					on error
						my logger(true, handlername, caller, "DEBUG", "No prior shows, skipping last-show fallbacks")
					end try
					set end of folder_fallbacks to alias "Volumes:"  -- tier 4: /Volumes fallback

					my logger(true, handlername, caller, "DEBUG", "Folder fallbacks: " & length of folder_fallbacks & " locations")
					set show_dir of temp_show_info to choose_folder_with_fallback(my cm(handlername, caller), "Select shows Directory for " & show_title of temp_show_info, folder_fallbacks) of LibScript

					-- Write-permission check; re-prompt at /Volumes if chosen folder isn't writable
					if show_dir of temp_show_info is not missing value then
						if update_folder(cm, show_dir of temp_show_info) of LibScript is false then
							my logger(true, handlername, caller, "WARN", "Chosen folder not writable, re-prompting")
							try
								set show_dir of temp_show_info to choose folder with prompt "Unable to write to that location. Select another:" default location alias "Volumes:"
							on error
								set show_dir of temp_show_info to missing value
							end try
						end if
					end if
				...
				set show_title of temp_show_info to stringToUtf8(cm, show_title of temp_show_info) of LibScript
				repeat with i from 1 to length of Show_info
				...
				repeatProgress(cm, 0.5, 3) of LibScript
			end repeat
		end repeat
	else
		return false
...
on record_start(caller, the_show_id, opt_show_length, force_update)
	...
	set i to HDHRShowSearch(my cm(handlername, caller), the_show_id) of LibScript
	set temp_show_end to short_date(my cm(handlername, caller), show_end of item i of Show_info, true, false) of LibScript
...
on getHDHR_Guide(caller, hdhr_device)
	...
	try
		...
		if hdhr_discover_temp is not equal to missing value then
			...
			set hdhr_guide_data to my hdhr_api(my cm(handlername, caller), "https://api.hdhomerun.com/api/guide.php?DeviceAuth=" & device_auth & "&Duration=" & Guide_hours)
			...
			my logger(true, handlername, caller, "INFO", "Updated Guide for " & hdhr_device & ", for " & Guide_hours & " hours")
			set progress completed steps to 1
		end if
	on error errmsg
		set progress completed steps to -1
...
on channel_guide(caller, hdhr_device, hdhr_channel, hdhr_time)
	...
	if hdhr_time is not "" then
		...
		set hdhr_proposed_time to datetime2epoch(my cm(handlername, caller), proposed_local_date) of LibScript
	...
	if Hdhr_device_list is not in {missing value, {}, 0, ""} then
		...
		repeat with i2 from 1 to length of Guide of temp_guide_data
			if (hdhr_proposed_time) is greater than or equal to getTfromN(StartTime of item i2 of Guide of temp_guide_data) of LibScript and (hdhr_proposed_time) is less than getTfromN(EndTime of item i2 of Guide of temp_guide_data) of LibScript then
				try
		...
		if time2show_next is less than or equal to 6 * hours and time2show_next is greater than or equal to -60 and show_active of item show_offset of Show_info is true or force_update is true then
			...
			if show_use_seriesid of item show_offset of Show_info is false then
				...
				if length of hdhr_response_channel is greater than 0 then
					...
					on error errmsg
						set hdhr_response_OriginalAirdate to epoch(cd) of LibScript
					...
					try
						...
						if (temp_show_time as number) is not equal to (show_time of item show_offset of Show_info as number) then
							...
							set show_time of item show_offset of Show_info to temp_show_time
					...
					try
						if show_url of item show_offset of Show_info is in {"", "?", false, "false"} then
							...
							set show_url of item show_offset of Show_info to add_record_url(my cm(handlername, caller), show_channel of item show_offset of Show_info, hdhr_record of item show_offset of Show_info) of LibScript
						end if
					on error errmsg
		...
		if length of Show_info is greater than 0 then
			repeat with i5 from 1 to length of temp_show_info
				if show_active of item i5 of temp_show_info is true then
					...
					try
						set show_time_OriginalAirdate of item i5 of temp_show_info to fixDate(my cm(handlername, caller), (show_time_OriginalAirdate of item i5 of temp_show_info)) of LibScript
					on error errmsg
...
on read_data(caller)
	...
	try
		...
		my logger(true, handlername, caller, "INFO", "Guide_hours (config): " & Guide_hours)
		--my logger(true, handlername, caller, "INFO", "Guide_hours (var): " & Guide_hours)
...
on recordingnow_main(caller)
	...
	if length of Show_info is greater than 0 then
		...
		return (Record_icon of Icon_record & " Recording" & return & stringlistflip(my cm(handlername, caller), recording_now_final, return, "string") of LibScript) as text
	else
...
on next_shows(caller)
	...
	if soonest_show is less than 9999999 then
		...
		repeat with i2 from 1 to length of Show_info
			try
				set temp_show_end to items 2 thru end of stringlistflip(my cm(handlername, caller), short_date(my cm(handlername, caller), show_end of item i2 of Show_info, false, false) of LibScript, " ", "list") of LibScript
			on error errmsg
			...
			if show_next of item i2 of Show_info is soonest_show_time and show_active of item i2 of Show_info is true then
				...
				short_date(my cm(handlername, caller), show_end of item i of Show_info, false, false) of LibScript
...
on showid2PID(caller, show_id, kill_pid, logging)
	...
	if show_id is "" then
		...
		if show_offset is greater than 0 then
			...
			set showid2PID_data_parsed to stringlistflip(my cm(handlername, caller), showid2PID_result, return, "list") of LibScript
...
on logger(logtofile, the_handler, caller, loglevel, message)
	...
	if loglevel is in Logger_levels then
		...
		set end of queued_log_lines to short_date(my cm(handlername, caller), current date, true, true) of LibScript & " " & Local_env & " " & loglevel & " " & the_handler & "(" & caller & ")" & " " & message
		if loglevel is in Logger_levels then
			try
...
on existing_shows(caller)
	...
	set showid2PID_result_list to stringlistflip(my cm(handlername, caller), showid2PID_result, return, "list") of LibScript
	if length of showid2PID_result_list is greater than 0 then
		try
			repeat with i from 1 to length of showid2PID_result_list
				set showid2PID_result_list_perline to stringlistflip(my cm(handlername, caller), item i of showid2PID_result_list, {" -H ", "show_id:"}, "list") of LibScript
				set temp_show_id to item 3 of showid2PID_result_list_perline
				set show_offset to HDHRShowSearch(my cm(handlername, caller), temp_show_id) of LibScript
...
on idle_change(caller, loop_delay, loop_delay_sec)
	...
	-- only run if we haven�t passed the end date yet
